...
#define US_TRIGGER_PULSE_US 10 // this is the esp32 singal to initate the process 
...
float hcsr_read_cm(void){
    int64_t guard_t0 = esp_timer_get_time(); //Starts a timer as soon as the esp32 starts 
...
float hcsr_read_cm_med(int samples, int delay_ms){
    // Only taking up to nine ultrasonic measurements to sort and return their median 
